...
use crate::font::{Char, Common, Font, Info, Padding, Page, Spacing};
...
impl FontBuilder {
    ...
    pub fn page<'a, A>(&mut self, attributes: &mut A) -> crate::Result<()>
    where
        A: Attributes<'a>,
    {
        self.pages.push(Page::load(attributes)?);
        Ok(())
    }
    ...
    where
        A: Attributes<'a>,
    {
        self.kernings.push(KerningPair::load(attributes)?);
        Ok(())
    }
}
...
implement_load!(
    ...
    (u8, 0x6, b"alphaChnl", alpha_chnl),
    (u8, 0x7, b"redChnl", red_chnl),
    (u8, 0x8, b"greenChnl", green_chnl),
    (u8, 0x9, b"blueChnl", blue_chnl)
);
